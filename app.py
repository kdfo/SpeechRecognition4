...
ICO = Image.open(IMG_DIR.joinpath("logo.png"))
LOGO = Image.open(IMG_DIR.joinpath("logo.png"))
...
def app():
    ...
    st.set_page_config(page_title="Text-to-Speech Converter", page_icon=ICO, layout="wide")
    ...
    This app allows you to convert plain text or SSML text into a WAV or MP3 audio file using Google's Cloud text-to-speech API. 
    Simply enter your plain or ssml text, choose from the various customization options, and then convert your text to an audio file.
    """
    )
    ...
    if not credentials_file:
        alert_box.error(f"""Could not find a valid JSON credential file! \n
        Please make sure the credential file is located in here: {STATIC_DIR.joinpath('credentials.json')}""")
        st.stop()
    ...
    text = text_input.text_area("Text to speak:", placeholder=f"Enter {input_type} here", key="input_text")
    ...
    if text:
        ...
        if send_button.button("Convert"):
            ...
            if result:
                ...
                alert_box.error("Something went wrong! Could not convert the text using the Google API!")
                st.stop()
